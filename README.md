...
Можно поднять сервер `python -m http.setver`. Он будет слушать на `0.0.0.0:8000`.
Затем проверять с помощью `curl`.

Весь код теперь в одном файле, потому что мне налоело копировать файлы в GNS туда сюда.  Я ленивый.
...
3. `REQUEST_METHOD <METHOD>`. Подходят под запрос определенного типа. Название метода также нечуствительно к регистру.
4. (Пока не работает) `RESPONSE_CODE <CODE>`. Только ответы с определенным кодом.
